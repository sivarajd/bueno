...
export function createRequestLogger(logger: Logger) {
  return async (ctx: unknown, next: () => Promise<unknown>) => {
    const context = ctx as { method: string; path: string; url: URL; getHeader: (name: string) => string | undefined; ip?: string; set: (key: string, value: unknown) => void; get: (key: string) => unknown };
    const start = Date.now();
...
export class PerformanceLogger {
   ...
   */
  stats(name: string): { count: number; min: number; max: number; avg: number; p99: number } | null {
