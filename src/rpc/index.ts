...
export interface RequestInterceptor {
  (config: RequestInterceptorContext): RequestInterceptorContext | Promise<RequestInterceptorContext>;
}

export interface ResponseInterceptor {
  (response: Response, context: InterceptorContext): Response | Promise<Response>;
}

export interface ErrorInterceptor {
  (error: Error, context: InterceptorContext): void | Response | Promise<void | Response>;
}
...
export interface RetryConfig {
  ...
  shouldRetry?: (response: Response, error: Error | null, attempt: number) => boolean;
}
...
class OptimisticStore {
  ...
  create<T>(
    ...
    optimisticData: T,
    previousData: T | undefined,
    callbacks?: { onRollback?: (prev: T | undefined) => void; onConfirm?: (data: T) => void }
  ): string {
...
function defaultKeyGenerator(method: HTTPMethod, url: string, body?: unknown): string {
...
const DEFAULT_RETRYABLE_ERRORS = ['ECONNRESET', 'ETIMEDOUT', 'ENOTFOUND', 'EAI_AGAIN'];
...
function defaultShouldRetry(
  ...
  if (error) {
    ...
    return DEFAULT_RETRYABLE_ERRORS.some(code => 
      error.message.includes(code) || error.name === code
    );
  }
...
export class RPCClient {
  ...
  constructor(options: RPCClientOptions) {
    ...
    this.retryConfig = {
      ...
      retryableErrors: options.retry?.retryableErrors ?? [...DEFAULT_RETRYABLE_ERRORS],
      onRetry: options.retry?.onRetry ?? (() => {}),
      shouldRetry: options.retry?.shouldRetry ?? ((response: Response, error: Error | null, attempt: number) => 
   ...
   * Make a POST request
   */
  async post<T>(path: string, body?: T, options?: RequestOptions): Promise<Response> {
   ...
   * Make a PUT request
   */
  async put<T>(path: string, body?: T, options?: RequestOptions): Promise<Response> {
   ...
   */
  async patch<T>(path: string, body?: T, options?: RequestOptions): Promise<Response> {
  ...
  private async request<T>(
    ...
    if (this.deduplicationConfig.enabled && !skipDeduplication) {
      ...
      const requestPromise = this.executeWithRetry<T>(method, url, body, options, dedupeKey);
  ...
  private async executeWithRetry<T>(
    ...
    while (state.attempt < maxAttempts) {
      ...
      try {
        const response = await this.executeRequest<T>(method, url, body, options, cacheKey);
        ...
        if (state.attempt < maxAttempts) {
          const delay = calculateDelay(state.attempt, { ...this.retryConfig, initialDelay, maxAttempts });
   ...
   */
  private shouldRetryResponse(response: Response, error: Error | null, attempt: number, maxAttempts: number): boolean {
    if (attempt >= maxAttempts) {
      return false;
    }
  ...
  private shouldRetryError(error: Error, attempt: number, maxAttempts: number): boolean {
    ...
    return this.retryConfig.shouldRetry(null as unknown as Response, error, attempt);
  }
  ...
  private async executeRequest<T>(
    ...
    for (const interceptor of this.requestInterceptors) {
      const interceptorContext: RequestInterceptorContext = { ...config, url, method };
  ...
  async optimisticPost<T = unknown, R = unknown>(
    path: string,
    body: T,
    options?: OptimisticOptions<R> & { query?: Record<string, string>; headers?: Record<string, string> }
  ): Promise<{ response: Response; rollbackId?: string }> {
  ...
  withBaseUrl(baseUrl: string): RPCClient {
    ...
    if (this.requestInterceptors.length > 0) interceptors.request = this.requestInterceptors;
    if (this.responseInterceptors.length > 0) interceptors.response = this.responseInterceptors;
    if (this.errorInterceptors.length > 0) interceptors.error = this.errorInterceptors;
