...
export class SchemaBuilder {
  ...
  // Indexes
  index(columns: string[], options?: { name?: string; unique?: boolean; type?: 'btree' | 'hash' | 'gin' | 'gist' }): this {
    this.indexes.push({
      columns,
      ...options,
    });
  ...
  foreignKey(columns: string[], reference: {
    table: string;
    columns: string[];
    onDelete?: 'CASCADE' | 'SET NULL' | 'RESTRICT' | 'NO ACTION';
    onUpdate?: 'CASCADE' | 'SET NULL' | 'RESTRICT' | 'NO ACTION';
 ...
 */
export function generateCreateTable(schema: TableSchema, driver: 'postgresql' | 'mysql' | 'sqlite' = 'postgresql'): string {
...
function mapColumnType(col: ColumnOptions, driver: 'postgresql' | 'mysql' | 'sqlite'): string {
...
function formatDefault(value: unknown, driver: 'postgresql' | 'mysql' | 'sqlite'): string {
 ...
 */
export type TypeScriptType<T extends ColumnType> = T extends 'serial' | 'integer'
  ? number
  : T extends 'bigint'
  ? bigint | string
  : T extends 'decimal'
 ...
 */
export function defineTable(name: string, define: (builder: SchemaBuilder) => void): TableSchema {
