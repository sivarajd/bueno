...
export function createTestRequest(
  ...
  // Build body
  let requestBody: string | ArrayBuffer | FormData | URLSearchParams | undefined;
  if (body !== undefined) {
...
export class AppTester {
  ...
  async request(path: string, options?: TestRequestOptions): Promise<TestResponse> {
    ...
    if (match.middleware && match.middleware.length > 0) {
      ...
      const response = await pipeline(context, async () => match.handler(context) as Response);
   ...
   * GET request helper
   */
  async get(path: string, options?: Omit<TestRequestOptions, 'method' | 'body'>): Promise<TestResponse> {
   ...
   * POST request helper
   */
  async post(path: string, body?: unknown, options?: Omit<TestRequestOptions, 'method' | 'body'>): Promise<TestResponse> {
   ...
   * PUT request helper
   */
  async put(path: string, body?: unknown, options?: Omit<TestRequestOptions, 'method' | 'body'>): Promise<TestResponse> {
   ...
   */
  async patch(path: string, body?: unknown, options?: Omit<TestRequestOptions, 'method' | 'body'>): Promise<TestResponse> {
 ...
 */
export function assertHeader(response: TestResponse, name: string, value?: string): void {
 ...
 * Assert redirect
 */
export function assertRedirect(response: TestResponse, location?: string): void {
