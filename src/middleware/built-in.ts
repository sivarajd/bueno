...
export function logger(options: LoggerOptions = {}): Middleware {
  ...
  return async (context: Context, next: () => Promise<Response>) => {
    ...
    // Log request
    if (format === 'json') {
      console.log(JSON.stringify({ type: 'request', method, path, timestamp: new Date().toISOString() }));
    } else {
...
export function securityHeaders(options: SecurityHeadersOptions = {}): Middleware {
  ...
  return async (context: Context, next: () => Promise<Response>) => {
    ...
    if (hsts) {
      ...
      const includeSubDomains = typeof hsts === 'object' ? hsts.includeSubDomains ?? true : true;
...
export function rateLimit(options: RateLimitOptions = {}): Middleware {
  const {
    ...
    handler = (ctx) => ctx.error('Too many requests, please try again later.', 429),
  } = options;
  ...
  return async (context: Context, next: () => Promise<Response>) => {
    ...
    context.setHeader('X-RateLimit-Remaining', String(Math.max(0, max - record.count)));
